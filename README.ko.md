...
- 클립보드 먹이기, 화면 캡처, 파일 선택, Finder 드롭, 채팅 입력을 위한 하나의 검증된 흐름
...
4. 펫을 오른쪽 클릭하거나 `Control+Option+Space`를 눌러 빠른 메뉴를 엽니다. 펫을 왼쪽 클릭하면 파일, 이미지, 텍스트 순서로 클립보드를 즉시 먹입니다. 키보드와 VoiceOver 사용자는 빠른 메뉴의 **클립보드에서 먹이기**를 사용할 수 있습니다.
...
- 왼쪽 드래그는 계속 펫 창을 이동하고 클립보드를 먹이지 않으며, 오른쪽 클릭은 창을 이동하지 않는지 확인합니다. 오른쪽 클릭은 빠른 메뉴를 열고 왼쪽 클릭은 클립보드를 먹여야 합니다.
