...
The DMG contains `YumYum Agent.app` and an `/Applications` symlink. Its portable checksum file contains `SHA-256  filename` in `shasum -a 256` format. Local and release packaging use the permanent `CFBundleIdentifier=io.github.kyu91.yumyumagent`. The release workflow rejects the exact old placeholder, empty or invalid identifiers, and any identity other than the permanent ID before signing or publishing.
