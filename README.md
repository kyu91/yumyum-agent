...
3. Grant Screen Recording, Accessibility, and Input Monitoring permission when prompted. This prompt reappears on every launch until all three are granted (or reopen it anytime from **Settings → Check Permissions**).
4. Right-click the pet to open the action bubble; left-click the pet to toggle the compact response bubble — the first click stages the clipboard into the chat draft, preferring files, then an image, then text, and opens the bubble with its inline composer ready, while a click while it is open just hides it (the draft stays put) and the next click reopens it without re-reading the clipboard. The clipboard-feed shortcut defaults to `Option+S`; click its recorder field in Settings and press any combination that includes at least one of ⌃/⌥/⌘ to rebind it, or press **Default** to reset it. Add any instruction, then press Return to send the staged clipboard content and instruction together.
